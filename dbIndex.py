...
from onnxImageSearch import load_onnx_model, encode_whole_dataset 
...
def indexDir(images_dir, encoder_path, signal=None):
    ...
    dir_dest = os.path.join(os.path.dirname(encoder_path), config.INDEXING_DIR, os.path.basename(images_dir))
    ...
    matching_df = pd.concat([matching_df, pd.DataFrame([{"images":images_dir, "embeddings":dest_embeddings, "infos":dest_df}])])
